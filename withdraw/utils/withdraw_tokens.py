...
def estimate_user_operation(address, cyber_address, contract_address, amount, chain_id, id_, proxy):
    hex_amount = '0' * (64 - len(hex(int(amount))[2:])) + hex(int(amount))[2:]
    json_data = {
        'jsonrpc': '2.0',
        'id': id_,
        ...
        'params': [
            {
                ...
                'callData': f'0xa9059cbb000000000000000000000000{config.to_address[2:]}{hex_amount}',
                'to': contract_address,
                'value': '0',
                'nonce': None,
                'maxFeePerGas': None,
    ...
    json_data = {
        ...
        'variables': {
            'input': {
                ...
                'type': 'TRANSFER_TOKEN',
                'readableTransaction': f'{{"recipient":"{config.to_address}","amount":"{int(amount) / config.token}","tokenIndex":0,"estimatedFee":{{"value":"{int(credits_) / config.token}","tier":"gasPriceFast"}},"token":{{{json.dumps(token_info, separators=(",", ":"))}}}}}',
            },
        },
...
def send_user_operation(user_operation, user_address, chain_id, id_, proxy):
    json_data = {
        'jsonrpc': '2.0',
        'id': id_,
...
def withdraw_tokens(user_private: str, user_address: str, cyber_address: str, auth: str, proxies: dict,
    ...
    if config.amount == 'all' and int(token_info['balance']) > 0:
        ...
        amount = config.amount * config.token
    else:
        return False

    last_hash = get_transaction_hash(auth, proxies)
    gas, credits_ = estimate_user_operation(user_address, cyber_address, contract_address, amount, chain_id, 0, proxies)
    ...
    send_user_operation(user_operation, user_address, chain_id, 1, proxies)
